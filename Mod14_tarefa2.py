...
print ()
mes = list(map(lambda x: x.upper(), (sys.argv[1:]))) #criando uma lista com os inputs em letra maiúscula


for m in mes:
    arquivo_ler = 'SINASC_RO_2019_' + m + '.csv'
    sinasc = pd.read_csv(arquivo_ler) #lendo cada arquivo
    max_data = sinasc.DTNASC.max()[:7]
    os.makedirs('./figs/'+max_data, exist_ok=True) #salvar as figuras nas pastas
    
    #printando os valores min e max das datas
